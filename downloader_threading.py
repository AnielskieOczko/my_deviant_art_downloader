...
def main():
    ...
    # load links/tasks to queue
    get_links(gallery="https://www.deviantart.com/kretualdo/gallery",q=tasks_list , img_limit=10)

    for i in range(my_acolyts):
        t = threading.Thread(target=my_acolytes, args=(tasks_list, download_lock))
        t.daemon = True
        t.start()
    tasks_list.join()
